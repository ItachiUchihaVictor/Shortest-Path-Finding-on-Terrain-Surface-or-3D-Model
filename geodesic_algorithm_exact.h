...
//        std::list<int> mcode;
        ...
        GeoNode(const int index,const float r){
            this->index=index;
            this->radius=r;
            this->chain=true; 
            this->parent=NULL;
...
stx::btree<int, GeoNode*> bplusparenttree;
stx::btree<int, GeoNode*> tem_tree;
namespace geodesic{
...
public:
	...
	void propagate_LA(std::vector<SurfacePoint>& sources,
                       std::vector<GeodesicAlgorithmExact *> * landmarks,
				   std::vector<SurfacePoint>* stop_points = NULL); //or after ensuring that all the stop_points are covered
	void propagate(std::vector<SurfacePoint>& sources,
   				   double max_propagation_distance = 0,			//propagation algorithm stops after reaching the certain distance from the source
	...
	void print_statistics(double&, double&);
...
private:
	...
	void update_list_and_queue_GB(list_pointer list,
							   ...
							   unsigned num_candidates,
                               SurfacePoint stop);
	void update_list_and_queue_LA(list_pointer list,
							   IntervalWithStop* candidates,	//up to two candidates
							   unsigned num_candidates,
                               std::vector<GeodesicAlgorithmExact *> * landmarks,
                               SurfacePoint stop);
    ...
    //IntervalQueue::size_type m_queue_max_size;			//used for statistics
...
inline bool GeodesicAlgorithmExact::range(std::vector<SurfacePoint>& sources,
				   double max_propagation_distance, GeoNode* geonode)
{
	set_sources(sources);
   ...
   //     std::cout<<"queue distance:"<<queue_distance<<"max_propagation_distance:"<<max_propagation_distance<<std::endl;
	    if(queue_distance > max_propagation_distance&&((!geonode)||(geonode&&parent)))break;
		...
		if(queue_distance > distance)
		{
//            std::cout<<"edge v0: "<<edge->v0()->id();
            if(distance<0.5*max_propagation_distance)tem_tree.erase(edge->v0()->id());
            chain=false;
            if(geonode&&!parent&&bplusparenttree.exists(edge->v0()->id())){
                parent=(*bplusparenttree.find(edge->v0()->id())).second;
            }
		}
		distance = interval_list(edge)->signal(edge->length());

		if(queue_distance > distance)
		{
            tem_tree.erase(edge->v1()->id());
            chain=false;
            if(geonode&&!parent&&bplusparenttree.exists(edge->v1()->id())){
                parent=(*bplusparenttree.find(edge->v1()->id())).second;
            }
		}
...
/*	for(unsigned i=0; i<m_edge_interval_lists.size(); ++i)
    ...
    if(bplusparenttree.size()==1)parent=(*bplusparenttree.begin()).second;
    if(geonode)geonode->setparent(parent);
    return chain;
}
inline void GeodesicAlgorithmExact::propagate_LA(std::vector<SurfacePoint>& sources,
                                        std::vector<GeodesicAlgorithmExact *> * landmarks, 
...
//		assert(min_interval->d() < GEODESIC_INF);
		...
		for(unsigned i=0; i<edge->adjacent_faces().size(); ++i)		//two possible faces to propagate
		{
			...
			if(num_propagated)
			{
				...
				update_list_and_queue_LA(interval_list(next_edge), 
									  candidates, 
									  num_propagated,
                                      landmarks,
                                      pstop);
...
inline void GeodesicAlgorithmExact::update_list_and_queue_LA(list_pointer list,
												IntervalWithStop* candidates,	//up to two candidates
												unsigned num_candidates,
                                                std::vector<GeodesicAlgorithmExact *> * landmarks,
                                                SurfacePoint pstop)
{
    ...
    double dist = 0.0; //std::max(pstop.distance(edge->v0())-edge->length(), 0.0 );
    double dt = 0.0;
    double de = 0.0;
    for(int i=0;i<landmarks->size();i++){
        (*landmarks)[i]->best_source(pstop, dt);
        SurfacePoint vpoint(edge->v0());
        (*landmarks)[i]->best_source(vpoint, de);
        if(dt < de+edge->length() && dt > de-edge->length())continue;
        dist = std::max(dist, std::min(std::abs(dt-de-edge->length()), std::abs(dt-de+edge->length())));
        //dist = std::max(dist, std::abs(dt-de+edge->length()));
    }
	...
	if(list->first() == NULL) 
	{
		...
		if(num_candidates == 1)
		{
			first = candidates;
			second = candidates;
			first->compute_min_distance_LA(first->stop(),dist);
		}
		else 
		{	
			...
			first->compute_min_distance_LA(first->stop(), dist);
			second->compute_min_distance_LA(second->stop(), dist);
		}
		...
		while(p != NULL && p->start() < q->stop() - local_epsilon)			//go through all old intervals
		{
			...
			if(N == 1)			
			{
				if(map[0]==OLD)	//if "p" is always better, we do not need to update anything)
				{
					...
					{
						previous->next() = p;
						previous->compute_min_distance_LA(p->start(), dist);
			...
			if(map[0]==OLD)	//finish previous, if any
			{
				...
				{
					previous->next() = p;
					previous->compute_min_distance_LA(previous->stop(), dist);
					m_queue.insert(previous);
					previous = NULL;
				}
				i_new[0] = p;
			...
			for(unsigned j=0; j<N; ++j)								//find "min" and add the intervals to the queue
			{
				...
				else
				{
					...
					current_interval->compute_min_distance_LA(current_interval->stop(), dist);					//compute minimal distance
                                                ...
                                                SurfacePoint pstop)
{
	...
	edge_pointer edge = list->edge();
	double const local_epsilon = SMALLEST_INTERVAL_RATIO * edge->length(); 
    double dist = std::max(std::max(pstop.distance(edge->v0())-edge->length(), pstop.distance(edge->v1())), 0.0 );
	if(list->first() == NULL) 
	{
		interval_pointer* p = &list->first();
		IntervalWithStop* first;
...
inline void GeodesicAlgorithmExact::print_statistics(double& num_intervals, double& num_edges)
{
	...
	for(unsigned i=0; i<m_edge_interval_lists.size(); ++i)
	{
        ...
        if(m_edge_interval_lists[i].number_of_intervals()) num_edges++;
	}
...
//    num_edges = m_edge_interval_lists.size();

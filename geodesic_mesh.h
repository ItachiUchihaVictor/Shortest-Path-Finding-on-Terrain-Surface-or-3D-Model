...
public:
    ...
    double m_xmin,m_width,m_ymin,m_height;
private:
...
//	for(unsigned i=0; i<num_faces; ++i){		//copy adjacent vertices to polygons/faces
//        geodesic::SurfacePoint p(&m_faces[i]);
//        v1.push_back(p.getx());
//        v2.push_back(p.gety());
 //       v3.push_back(p.getz());
//		for(unsigned j=0; j<3; ++j){
//            f1.push_back(num_vertices+i);
//            f2.push_back(m_faces[i].adjacent_vertices()[(j+1)%3]->id());
//            f3.push_back(m_faces[i].adjacent_vertices()[(j+2)%3]->id());
//        }
 //   }
//       std::ofstream sp("SteinerPoints.txt", std::ios::out );
       // sp //<< "OFF" << std::endl
